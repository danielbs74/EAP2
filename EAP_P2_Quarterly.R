...
compustat <- ccmfund %>%
  transmute(
    ...
    revt = as.numeric(revtq),           # Total Revenue
    cogs = as.numeric(cogsq),           # Cost of goods sold
    ...
    be = as.numeric(ceqq),             # total common/ordinary equity
    at = as.numeric(atq),               # total assets
...
month(compustat$date) = ifelse((compustat$date %>% month) < 3, 3, compustat$date %>% month)
month(compustat$date) = ifelse((compustat$date %>% month) > 3 & (compustat$date %>% month) < 6, 6, compustat$date %>% month)
month(compustat$date) = ifelse((compustat$date %>% month) > 6 & (compustat$date %>% month) < 9, 9, compustat$date %>% month)
month(compustat$date) = ifelse((compustat$date %>% month) > 9, 12, compustat$date %>% month)
...
compustat$be = ifelse(compustat$be <= 0, NA, compustat$be)
...
day(stocks$date) = 1
compustat$date = compustat$date %m+% months(4)
...
stocks = stocks[stocks$date < '2020-12-31' & stocks$date > '1975-06-30',]
...
if(!file.exists('stocksFAq.rds')){
  ...
  saveRDS(stocksFA, 'stocksFAq.rds')
  
} else { 
  stocksFA = readRDS('stocksFAq.rds')
}
